...
let LoyaltyService = class LoyaltyService {
    firebaseService;
    aiService;
    constructor(firebaseService, aiService) {
        ...
        if (ordersProfile) {
            ...
            if (!doc.exists || !data ||
                ...
                await db.collection('loyaltyCustomers').doc(uid).set({
                    ...
                    joinDate: ordersProfile.joinDate || (data?.joinDate?.toDate?.() || firestore_1.FieldValue.serverTimestamp()),
                    lastPurchase: ordersProfile.lastPurchase || (data?.lastPurchase?.toDate?.() || firestore_1.FieldValue.serverTimestamp()),
    ...
    async getNameEmailFromOrders(uid) {
        try {
            const db = this.getDb();
            const snap = await db
    ...
    async getCustomerByEmailAndSync(email) {
        const db = this.getDb();
        ...
        await db.collection('loyaltyCustomers').doc(uid).set({
            ...
            totalPoints,
            level,
            joinDate: sorted[0]?.createdAt || firestore_1.FieldValue.serverTimestamp(),
            lastPurchase: sorted[sorted.length - 1]?.createdAt || firestore_1.FieldValue.serverTimestamp(),
            purchaseCount,
    ...
    async updateCustomerProfile(uid, updates) {
        const db = this.getDb();
        await db.collection('loyaltyCustomers').doc(uid).set({ ...updates, updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
    }
    async calculatePoints(orderAmount) {
        return Math.floor(orderAmount);
    }
    async addPurchase(uid, orderAmount, orderId) {
        const db = this.getDb();
        ...
        if (!profile) {
            ...
            await db.collection('loyaltyCustomers').doc(uid).set({
                ...
                lastPurchase: firestore_1.FieldValue.serverTimestamp(),
                purchaseCount: 1,
    ...
    async buildProfileFromOrders(uid) {
        const db = this.getDb();
        const ordersSnapshot = await db
            .collection('CustomerOrders')
            .where('userId', '==', uid)
    ...
    async getTopCustomers(limit = 10) {
        ...
        const customers = await Promise.all(Array.from(ordersMap.entries()).map(async ([uid, orderData]) => {
    ...
    async getAnalytics() {
        ...
        try {
            ...
            loyaltySnapshot.docs.forEach((doc) => {
                ...
                const level = orderData ? orderData.level : (data.level || 'Silver');
    ...
    async predictChurnRisk(uid) {
        const profile = await this.getCustomerProfile(uid);
        if (!profile)
            return 100;
        return this.aiService.predictChurnRisk(profile);
    }
    async generatePersonalizedOffers(uid) {
        const profile = await this.getCustomerProfile(uid);
        if (!profile)
            return [];
    ...
    async syncFromOrders() {
        ...
        for (const [uid, orders] of Object.entries(ordersByUser)) {
            ...
            await db.collection('loyaltyCustomers').doc(uid).set({
                ...
                totalPoints,
                level,
                joinDate: oldest.createdAt || firestore_1.FieldValue.serverTimestamp(),
                lastPurchase: latest.createdAt || firestore_1.FieldValue.serverTimestamp(),
                purchaseCount,
                averageOrderValue: purchaseCount ? totalSpent / purchaseCount : 0,
                refillConsistency: 0,
                engagementScore: 50,
    ...
    async cleanupEmailKeyedLoyaltyDocs() {
        const db = this.getDb();
